#!D:\softwares\miniconda\envs\yixin\python.exe
...
import pdfminer_for_img.high_level
from pdfminer_for_img.layout import LAParams
from pdfminer_for_img.utils import AnyIO
...
def extract_text(
    ...
    for fname in files:
        with open(fname, "rb") as fp:
            pdfminer_for_img.high_level.extract_text_to_fp(fp, **locals())
    return outfp
...
def maketheparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--files", type=str, default='', help="One or more paths to PDF files.")

    parser.add_argument(
        "--version", "-v", action="version",
        version="pdfminer.six v{}".format(pdfminer_for_img.__version__))
    parser.add_argument(
        "--debug", "-d", default=False,
        help="Use debug logging level.")
    parser.add_argument(
        "--disable-caching", "-C", default=False,
    ...
    la_params.add_argument(
        "--no-laparams", "-n", default=False,
        help="If layout analysis parameters should be ignored.")
    la_params.add_argument(
        "--detect-vertical", "-V", default=False,
    ...
    output_params.add_argument(
        "--strip-control", "-S", default=False,
        help="Remove control statement from text. "
             "Only used when output_type is xml.")
    return parser


def extract_imgs(pdf_path, pic_dir):
    P = maketheparser()
    A = P.parse_args()
    A.files = [pdf_path]
    A.output_dir = pic_dir
    A.outfile = 'pdfminer_for_img/tmp.txt'
    ...
    convert_img(pic_dir, remove=True)

    return 0


def convert_img(path, remove=False):
    '''
    将bmp格式图片转为jpg图片
    :param path:
    :param remove:
    ...
    for root, _, files in os.walk(path):
        for file in files:
            if file.endswith('bmp'):
                ...
                try:
                    img.save(os.path.join(root, basename + '.jpg'))
                    if remove:
...
if __name__ == '__main__':
    extract_imgs(r'sample_data/e.pdf', 'sample_data/dd')
