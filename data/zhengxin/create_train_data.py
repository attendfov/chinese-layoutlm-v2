...
def split_train_data():
    ...
    train, dev = train_test_split(document, test_size=0.2, random_state=42)
    ...
    for i, doc in enumerate(train):
        for j, line in enumerate(doc['document']):
            if line['label'] == '' or line['label'] not in all_labels:
                if line['label'] != '' and line['label'] not in all_labels:
                    print(line['label'])
