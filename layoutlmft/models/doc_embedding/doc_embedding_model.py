...
class docEmbeddingModel(LayoutLMv2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.layoutlmv2 = LayoutLMv2Model(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(
        ...
        for batch in [batch1, batch2]:
            ...
            outputs, final_attention_mask = self.layoutlmv2(
                input_ids=input_ids,
                bbox=bbox,
                image=image,
                attention_mask=attention_mask,
            ...
            input_mask_expanded = final_attention_mask.unsqueeze(-1).expand(sequence_output.size()).float()
            sum_embedding = torch.sum(sequence_output * input_mask_expanded, dim=1)

            sum_mask = torch.sum(input_mask_expanded, dim=1)
            sum_mask = torch.clamp(sum_mask, min=1e-9)

            reps.append(sum_embedding / sum_mask)
        ...
        if labels is not None:
            ...
            return {'loss':loss, 'reps':reps, 'preds':output}
        else:
